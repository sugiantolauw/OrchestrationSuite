...
- **Threshold [PENDING — analyst-set]:** $40 per head if every attendee is internal, $80 if any attendee is external. External = `Company` or `External ID` present. **[PENDING]** Confirm the internal/external rule.
