...
For the T&E Skill (SKILL-001), pulls real methodology content directly from
the existing test catalogue, file registry, and category order — so the
viewer reflects exactly what the app runs.
...
def get_methodology(skill_id: str) -> dict:
    """Return methodology metadata for a given skill_id."""
    if skill_id == "SKILL-001":
        return _tne_methodology()
    return _stub_methodology(skill_id)


# ─── T&E Skill methodology (real, pulled from live catalogue) ────────────────

def _tne_methodology() -> dict:
    from src.test_catalogue import TEST_CATALOGUE, CATEGORY_ORDER
    from src.data_loader import FILE_REGISTRY

    # Data sources: pick the ones referenced by the catalogue + a few core ones
    data_source_keys = sorted({t["source_file_key"] for t in TEST_CATALOGUE
                               if t.get("source_file_key") in FILE_REGISTRY})
    data_sources = []
    for key in data_source_keys:
        reg = FILE_REGISTRY[key]
        data_sources.append({
            "key": key,
