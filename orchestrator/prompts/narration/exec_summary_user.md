...
top_findings[0] is this run's biggest story; where its own `placeholders` list gives you a
striking figure for it (for example the share of a population affected), name that specific
finding and cite that number using a placeholder from ITS OWN `placeholders` list -- never invent
one, and never use a placeholder listed for a different finding. Do not merely restate the finding
count and the headline in general terms.
